...
export default function GovtMonitors() {
    ...
    fetch(endpoint, {
      ...
      .then((data) => {
        if (isEditMode) {
          setGovMonData(
            govMonData.map((item) =>
              item.id === newGovMon.id ? data.data : item
            )
          );
  ...
  const handleDelete = (id) => {
    if (
      window.confirm("Are you sure you want to delete this government monitor?")
    ) {
          ...
          <div className="bg-white p-6 rounded-lg w-full max-w-md">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-semibold">
                {isEditMode
                  ? "Edit Government Monitor"
                  : "Add Government Monitor"}
              </h2>
              <button
                onClick={closeModal}
