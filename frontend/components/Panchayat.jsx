...
export default function Panchayat() {
  ...
  const openModal = (isEdit = false, data = null) => {
    ...
    if (isEdit && data) {
      ...
      setEnvFields(
        envDataArray.length > 0 ? envDataArray : [{ key: "", value: "" }]
      );
      setNewPanchayat({
        id: data.id,
        name: data.name,
  ...
  const handleSubmit = (e) => {
    ...
    fetch(endpoint, {
      ...
      .catch((error) => {
        console.error(
          `Error ${isEditMode ? "updating" : "adding"} panchayat data:`,
          error
        );
        toast.error(
          `Error ${isEditMode ? "updating" : "adding"} panchayat data!`
  ...
  const handleDelete = (id) => {
    if (
      window.confirm("Are you sure you want to delete this panchayat data?")
    ) {
                ...
                filteredData.map((data) => (
                  <tr key={data.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      {data.address.village}, {data.address.street},{" "}
                ...
                {envFields.map((field, index) => (
                  <div key={index} className="flex mb-2">
                    <input
                      ...
                      value={field.key}
                      onChange={(e) =>
                        handleEnvFieldChange(index, "key", e.target.value)
                      }
                    ...
                    <input
                      ...
                      value={field.value}
                      onChange={(e) =>
                        handleEnvFieldChange(index, "value", e.target.value)
                      }
