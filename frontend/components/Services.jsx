    ...
    fetch('http://localhost:5000/fetch_citizen_data')
      ...
      .then((data) => {
        ...
        if(visitorrole === 'panchayat'){
          setCitizensData(data.filter((item) => item.panchayat_id === visitorpanchayat));
        }
      })
      .catch((error) => {
  ...
  useEffect(() => {
    fetch("http://localhost:5000/fetch_services")
      ...
      .then((data) => {
        ...
        if(visitorrole === 'panchayat'){
          setServicesData(data.filter((item) => item.panchayat_id === visitorpanchayat));
        }
        setLoading(false);
      })
      .catch((error) => {
  ...
  const handleDelete = (id) => {
    if (window.confirm("Are you sure you want to delete this service?")) {
      fetch(`http://localhost:5000/delete_service/${id}`, {
        ...
        .then((res) => res.json())
        .then(() => {
          setServicesData(servicesData.filter((item) => item.service_id !== id));
