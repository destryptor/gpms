...
export default function PanchayatMembers() {
    ...
    try {
      if (modalMode === 'add') {
        ...
        if (response.ok) {
          ...
          // Refresh data instead of manually updating state
          fetchAllData();
      ...
      toast.error(`Error ${modalMode === 'add' ? 'adding' : 'updating'} member: ${error.message}`);
    }
  };
  ...
  const handleDelete = async (citizenId) => {
    if (window.confirm('Are you sure you want to delete this member?')) {
      try {
        const response = await fetch(`${baseUrl}/delete_citizen_panchayat/${citizenId}`, {
          method: 'DELETE',
        });
        ...
        if (response.ok) {
          ...
          setPanchayatMembersData(panchayatMembersData.filter(
            ...
            <tbody>
              {loading ? (
                ...
                filteredData.map((data, index) => (
                  <tr key={data.citizen_id || index} className="hover:bg-gray-50">
                    <td className="px-6 py-4">{data.citizen_id}</td>
                    <td className="px-6 py-4">{getCitizenName(data.citizen_id)}</td>
                    <td className="px-6 py-4">{data.panchayat_id}</td>
                    <td className="px-6 py-4">{getPanchayatName(data.panchayat_id)}</td>
          ...
          <div className="bg-white p-6 rounded-lg w-full max-w-md">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-semibold">
                {modalMode === 'add' ? 'Add Panchayat Member' : 'Edit Panchayat Member'}
              </h2>
              <button
            ...
            <form onSubmit={handleSubmit}>
              <div className="mb-4">
                ...
                <select
                  ...
                  required
                  disabled={modalMode === 'edit'} // Can't change citizen_id in edit mode since it's the primary key
                >
