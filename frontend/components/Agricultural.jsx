...
export default function Agricultural() {
	...
	useEffect(() => {
		if (visitorpanchayat && (visitorrole === 'panchayat' || visitorrole === 'citizen')) {
			fetchAgricultureData();
      fetchCitizenData();
		}
		...
		fetch('http://localhost:5000/fetch_agriculture_data')
			...
			.then((data) => {
				...
				data.forEach((item) => {
					...
					const crops = Array.isArray(item.crops_grown) ? item.crops_grown : item.crops_grown.split(',').map((crop) => crop.trim());
	...
	const fetchCitizenData = () => {
        ...
        if (visitorrole === 'panchayat') {
          console.log('panchayat id: ', visitorpanchayat);
          setCitizenData(data.filter((citizen) => citizen.panchayat_id === visitorpanchayat));
        }
			})
			.catch((error) => {
	...
	const handleCitizenSelect = (e) => {
		...
		const selectedCitizen = citizenData.find((citizen) => citizen.id === selectedId);
    ...
    // console.log(submissionData);
		...
		const endpoint = modalMode === 'add' ? 'http://localhost:5000/add_agriculture_data' : 'http://localhost:5000/update_agriculture_data';
		...
		<div className='p-6 flex-1 bg-white rounded-lg h-full overflow-y-auto'>
			<div className='flex flex-row justify-between items-center'>
				<div className='flex flex-row text-center justify-center space-x-2'>
					<span className='text-lg font-semibold text-gray-700'>Agricultural Data</span>
				</div>
				{visitorrole === 'admin' || visitorrole === 'panchayat' && (
					<button className='py-2 px-4 bg-[#000000] font-medium text-sm text-white rounded-lg cursor-pointer' onClick={() => handleOpenModal('add')}>
						Add Data
					</button>
				)}
			</div>
			...
			{visitorrole !== 'panchayat' && visitorrole !== 'citizen' && (
				<div className='mt-4 mb-4 flex flex-row justify-between items-center w-full gap-2'>
					<div className='relative w-full'>
						...
						<input type='text' className='block w-full p-2 pl-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500' placeholder='Search by panchayat name...' value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
					</div>
					<button onClick={searchByPanchayat} className='py-1.5 px-2 bg-black cursor-pointer text-white rounded-lg'>
						Search
					</button>
				</div>
			)}
			...
			{visitorrole === 'government_monitor' && (
				<>
					{Object.keys(cropmap).length > 0 && (
						<div className='mt-6'>
							<h2 className='text-lg font-semibold text-gray-700 mb-2'>Crop-wise Area Distribution</h2>
									...
									{loading ? (
										...
										filteredData.map((data) => (
											<tr key={data.agriculture_id} className='hover:bg-gray-50'>
												...
												<td className='px-6 py-4'>{data.address ? `${data.address.street}, ${data.address.city}, ${data.address.zipcode}` : 'N/A'}</td>
												<td className='px-6 py-4'>{data.area_in_hectares}</td>
												<td className='px-6 py-4'>{Array.isArray(data.crops_grown) ? data.crops_grown.join(', ') : data.crops_grown}</td>
												...
												{visitorrole === 'admin'  && (
													<td className='px-6 py-4'>
														<div className='flex space-x-2'>
															<button onClick={() => handleOpenModal('update', data)} className='font-medium text-blue-600 hover:text-blue-800 cursor-pointer'>
																Edit
															</button>
															<button onClick={() => handleDelete(data.id)} className='font-medium text-red-600 hover:text-red-800 cursor-pointer'>
																Delete
															</button>
														</div>
													</td>
			...
			{isModalOpen && (
				<div className='fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50'>
					<div className='bg-white p-6 rounded-lg w-full max-w-3xl'>
						<h2 className='text-xl font-semibold mb-4'>{modalMode === 'add' ? 'Add New' : 'Update'} Agricultural Data</h2>

						<form onSubmit={handleSubmit}>
							{/* Owner Selection */}
							<div className='mb-4'>
								<label className='block text-gray-700 text-sm font-bold mb-2'>Owner</label>
								<select className='w-full px-3 py-2 border rounded-lg' value={formData.citizen_id} onChange={handleCitizenSelect} required>
							...
							<div className='mb-4'>
								...
								<input type='text' name='address.street' value={formData.address.street} onChange={handleInputChange} className='w-full px-3 py-2 border rounded-lg' required />
							</div>

							<div className='grid grid-cols-2 gap-4 mb-4'>
								<div>
									<label className='block text-gray-700 text-sm font-bold mb-2'>City</label>
									<input type='text' name='address.city' value={formData.address.city} onChange={handleInputChange} className='w-full px-3 py-2 border rounded-lg' required />
								</div>
								<div>
									<label className='block text-gray-700 text-sm font-bold mb-2'>Zipcode</label>
									<input type='text' name='address.zipcode' value={formData.address.zipcode} onChange={handleInputChange} className='w-full px-3 py-2 border rounded-lg' required />
								</div>
							</div>

							{/* Area in Hectares */}
							<div className='mb-4'>
								<label className='block text-gray-700 text-sm font-bold mb-2'>Area in Hectares</label>
								<input type='number' name='area_in_hectares' value={formData.area_in_hectares} onChange={handleInputChange} className='w-full px-3 py-2 border rounded-lg' required step='0.01' />
							</div>

							{/* Crops Grown */}
							<div className='mb-4'>
								<label className='block text-gray-700 text-sm font-bold mb-2'>Crops Grown (comma separated)</label>
								<input type='text' name='crops_grown' value={formData.crops_grown} onChange={handleInputChange} className='w-full px-3 py-2 border rounded-lg' required placeholder='wheat, rice, corn' />
							</div>

							<div className='flex justify-end gap-2'>
								<button type='button' onClick={handleCloseModal} className='py-2 px-4 bg-gray-200 text-gray-800 rounded-lg cursor-pointer hover:bg-gray-300'>
									Cancel
								</button>
								<button type='submit' className='py-2 px-4 bg-black text-white rounded-lg cursor-pointer hover:bg-gray-800'>
